...
上圖中，假設我們有一個 pool ，其中有 4 個數據集叫 d1\~d4 ，然後數據集 d1 有兩個快照 s1 和
s2 。
...
除了回滾操作，還可以直接只讀訪問到快照中的文件。 ZFS 的數據集中有個隱藏文件夾叫 ".zsh"
，所以如果只想回滾一部分文件，可以從 ".zsh/snapshot/SNAPSHOT-NAME" 中把需要的文件複製出來。
...
而這個快照的內容已經在服務器上所以不需要保留在筆記本中。有了快照之後，每次將定期的新快照 send
到服務器之後，就可以把這個快照轉化成書籤，節省存儲開銷。

檢查點（checkpoint）
++++++++++++++++++++++++++++++++++++

這也是 ZFS 的新特性ZFS on Linux 分支從 v0.6.4 開始支持創建檢查點。

簡而言之，檢查點可以看作是整個 pool 級別的快照，使用檢查點能快速將整個 pool 都恢復到上一個狀態。
這邊有篇文章介紹 `ZFS checkpoint 功能的背景、用法和限制 <https://sdimitro.github.io/post/zpool-checkpoint/>`_
，可以看出當 pool 有檢查點的時候很多 pool wide 的功能會受影響（不能刪除 vdev ，不能處於
degraded 狀態，不能 scrub 到當前 pool 已經釋放而檢查點還在引用的數據塊），
於是檢查點功能設計上更多是給系統管理員準備的在調整 ZFS pool 時的後悔藥，
日用狀態下應該刪除掉所有檢查點。

ZFS 的概念與 btrfs 概念的對比
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

先說書籤和檢查點，因爲這是兩個 btrfs 完全沒有的功能。

書籤功能完全圍繞 ZFS send 的工作原理，
而 ZFS send 位於 ZFS 設計的 DSL (dataset and snapshot layer) 層面，甚至不關心它 send
的快照的數據是來自文件系統還是 zvol 。

可以看出和 btrfs 相比， ZFS 的快照有更多限制，而 ZFS 快照允許的操作都可以在 btrfs 中通過
文件系統操作進行