...
基於段的 GC 還有一個顯著缺陷，需要複製搬運仍然被引用到的塊，這不光會增加設備寫入，
...
放在存儲設備期待的 FAT 所在位置減少這種指針調整導致的寫入。

不過基於段的 GC 也有一些好處，它不需要複雜的文件系統設計，不需要特殊構造的指針，
就能很方便地支持大量快照。一些日誌結構文件系統比如 NILFS 用這一點支持了「連續快照（continuous
snapshots）」，每次文件系統提交都是創建一個快照，用戶可以手動標記需要保留哪些快照，
GC 算法則排除掉用戶手動標記的快照之後，根據快照創建的時間，先從最老的未標記快照開始回收。
即便如此， GC 的開銷（CPU時間和磁盤讀寫帶寬）仍然是 NILFS
最爲被人詬病的地方，是它難以被廣泛採用的原因。

WAFL 早期使用的可用空間位圖數組
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

從日誌結構文件系統的 GC 的困境中可以看出，文件系統級別實際更合適的不是運行期的 GC
，而是在創建快照時或者寫入數據時就記錄下快照使用的數據塊，這也是 WAFL 早期實現快照的設計思路。

WAFL 早期記錄快照佔用數據塊的思路也很「暴力」，傳統文件系統一般有個「位圖（bitmap）」用一個 bit
記錄一個數據塊是否佔用，靠掃描位圖來尋找可用空間和已用空間。 WAFL
的設計早期中考慮既然需要支持快照，那就把記錄數據塊佔用情況的位圖，變成一個快照的數組。
...
.. ditaa::
             ...
             /--------+--------+--------+--------+--------+--------+--------\
 filesystem  |cGRE  1 |cGRE  2 |cGRE  3 |cGRE  4 |cGRE  5 |      6 |      7 |
             +--------+--------+--------+--------+--------+--------+--------+
 snapshot1   |cGRE  1 |      2 |cGRE  3 |cGRE  4 |cGRE  5 |cGRE  6 |      7 |
             +--------+--------+--------+--------+--------+--------+--------+
 ...         |cGRE  1 |      2 |cGRE  3 |      4 |cGRE  5 |      6 |cGRE  7 |
             +--------+--------+--------+--------+--------+--------+--------+
 snapshot255 |      1 |      2 |      3 |      4 |      5 |      6 |      7 |
             \--------+--------+--------+--------+--------+--------+--------/

上面每個單元格都是一個 bit ，表示某個快照有沒有引用某個數據塊。