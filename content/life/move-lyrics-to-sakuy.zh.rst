...
:code:`<ruby>` 標籤的振假名( :ruby:`振|ふ` り :ruby:`仮名|かな` )。
...
這個 :ruby:`行內角色|inline role` 但是大量用也不方便）。這個博客一開始用
`Pelican 重寫主題的時候 <{filename}/tech/redesign-pelican-theme.zh.rst#bootstrapify-twitter-bootstrap-rst-directives>`_
